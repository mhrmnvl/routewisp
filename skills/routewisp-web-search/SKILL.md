---
name: routewisp-web-search
description: Web and X search via routewisp /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity / Xquik. Use when the user wants to search the web, find articles, or search public X posts.
---

# routewisp — Web Search

Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See `skills/routewisp/SKILL.md` for setup.
...
curl -X POST $NINEROUTER_URL/v1/search \
  ...
  -d '{"model":"tavily","query":"AI routing gateways","max_results":5}'
```
...
Add the Xquik API key via `node bin/routewisp.mjs providers add`. Xquik charges 1 credit per returned post. Continue a search by passing `pagination.next_cursor` as `provider_options.cursor`.
