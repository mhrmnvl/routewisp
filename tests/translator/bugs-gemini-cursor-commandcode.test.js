...
describe("OpenAI → Gemini", () => {
  ...
  it("tool_result content with $-prefixed JSON-Schema keys is sanitized", () => {
    const out = O2G({
      messages: [
        ...
        { role: "tool", tool_call_id: "c1", content: JSON.stringify({ $ref: "#/defs/x", ok: 1 }) },
      ],
    });
    const json = JSON.stringify(out);
    expect(json).not.toContain('"$ref"');
    expect(json).toContain('"_ref"');
  });
});
