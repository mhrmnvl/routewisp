# 9Router Architecture (upstream doc — read with care in this fork)

> **routewisp note:** this file is upstream 9router's own architecture doc,
> kept because most of it still describes `open-sse/` accurately (vendored
> here essentially unmodified). It also still describes the Next.js
> dashboard, cloud sync, and other features that **do not exist in this
> fork** — see `CLAUDE.md` and `SETUP.md` for what routewisp actually is.
> Treat anything below about the dashboard, `db.json`, or cloud sync as
> upstream history, not this repo's behavior.

_Last updated: 2026-02-06 (upstream)_
