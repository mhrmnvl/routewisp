...
function sanitizeFunctionResponseValue(value) {
  if (Array.isArray(value)) return value.map(sanitizeFunctionResponseValue);
  if (value && typeof value === "object") {
    const out = {};
    for (const [key, val] of Object.entries(value)) {
      out[key.startsWith("$") ? `_${key.slice(1)}` : key] = sanitizeFunctionResponseValue(val);
    }
    return out;
  }
  return value;
            ...
            for (const fid of toolCallIds) {
              ...
              toolParts.push({
                functionResponse: {
                  ...
                  response: { result: sanitizeFunctionResponseValue(parsedResp) }
                }
              });
            }
